...
(function($) {
	...
	_.extend(SFDC.SObjectView.prototype, {
    	initialize: function(options) {
    		...
    		SFDC.View.prototype.initialize.apply(this, arguments);
            _self.viewContainer = options.viewContainer;
    	...
    	prepare: function() {
    		...
    		if (_self.sobject) {
                // If sobject type has changed that re-create view helper
                var sobjectTypeInst = getSObjectType(_self.sobject);
                if (!this._viewHelper || this._viewHelper.sobjectType != sobjectTypeInst)
                    this._viewHelper = new SObjectViewHelper(this.sobject);

                // Re-instantiate the model for new attributes
    	...
    	render: function() {
    		...
    		_self._statusMonitor.done(function(templateInfo) {
                ...
                var templateModel = new SObjectViewModel(_self.model, templateInfo.fieldInfos);
    			//var templateInst = template.createInstance(templateModel);
                //var templateHost = _self.el.shadowRoot;
                //while (templateHost.firstChild) templateHost.removeChild(templateHost.firstChild);
                $(_self.viewContainer).empty().append(template.createInstance(templateModel));
                _self.el.model = templateModel; //FIXIT: Doesn't seem like the right way to do this
                setTimeout(function() { _self.trigger('afterRender'); }, 0);
    		});
