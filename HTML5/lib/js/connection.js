...
sforce.Transport = function(url) {
    ...
    this.send = function (envelope, callback, async, timeout) {
        ...
        var url = (sforce.connection.useProxy) ? (location.protocol + "//" + location.hostname + "/services/proxy") : this.url;
        sforce.internal._connections.push(holder);
        this.connection.open("POST", url, async);
        this.connection.setRequestHeader("Content-Type", "text/xml; charset=UTF-8");
        this.connection.setRequestHeader("SOAPAction", "\"\"");
        this.connection.setRequestHeader("Accept", "text/xml");
        this.connection.setRequestHeader("User-Agent", "SFAJAX 1.0");
        if (sforce.connection.useProxy) {
            this.connection.setRequestHeader("SalesforceProxy-Endpoint", this.url);
            this.connection.setRequestHeader("SalesforceProxy-SID", sforce.connection.sessionId);
        }
...
sforce.Connection.prototype.init = function(sessionId, serverUrl, useProxy) {
